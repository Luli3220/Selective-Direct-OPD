...
class BaseCheckpointManager:
    ...
    def remove_previous_save_local_path(self, path):
        if isinstance(path, str):
            path = [path]
        # Make sure no async HDFS upload is still reading from this directory before
        # we delete it. Disabled / no-op uploader returns immediately.
        try:
        ...
        for p in path:
            ...
            if uploader is not None:
                # wait_for_path is a no-op when the uploader is disabled.
                uploader.wait_for_path(abs_path)
                uploader.wait_for_path(p)
