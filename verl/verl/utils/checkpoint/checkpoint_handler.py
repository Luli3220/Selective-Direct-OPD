...
class CheckpointHandler:
    ...
    def save_checkpoint(self, step):
        ...
        # Copy to HDFS if configured (async; falls back to sync inside uploader when disabled)
        if self.rank == 0 and self.default_hdfs_dir:
            remote_global_step_folder = os.path.join(
                self.default_hdfs_dir, f"global_step_{step}"
            )
            get_uploader().enqueue_copy(local_global_step_folder, remote_global_step_folder)
