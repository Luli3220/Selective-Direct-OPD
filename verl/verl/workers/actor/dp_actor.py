            ...
            if self.use_remove_padding:
                ...
                if self.use_fused_kernels and not need_logits:
                    ...
                    if need_topk:
                        ...
                        log_probs = logits_rmpad.gather(
                            dim=-1, index=input_ids_rmpad_rolled.unsqueeze(-1)
                        ).sub(log_norm).squeeze(-1)
                    else:
                    ...
                    if need_topk:
                        ...
                        topk_log_probs = logits_rmpad.gather(dim=-1, index=topk_ids).sub(log_norm)
                ...
                if self.use_fused_kernels and not need_logits:
                    ...
                    if need_topk:
                        ...
                        topk_log_probs = logits.gather(dim=-1, index=topk_ids).sub(log_norm)
            ...
            for batch_idx, mini_batch in enumerate(mini_batches):
                ...
                for micro_batch in micro_batches:
                    ...
                    if js_token_filter_enabled:
                        ...
                        valid_token_count = response_mask.sum().clamp_min(1)
