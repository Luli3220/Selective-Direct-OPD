...
from verl.utils.async_hdfs_uploader import get_uploader
...
def _build_per_prompt_balanced_indices(num_prompts: int, repeat_times: int, dp_size: int) -> list[int] | None:
    ...
    if num_prompts <= 0 or repeat_times <= 0 or dp_size <= 0:
        return None
    if repeat_times % dp_size != 0:
        return None

    per_rank_repeat = repeat_times // dp_size
    indices = []
    for rank in range(dp_size):
        rank_start = rank * per_rank_repeat
        for prompt_idx in range(num_prompts):
            prompt_start = prompt_idx * repeat_times
            indices.extend(range(prompt_start + rank_start, prompt_start + rank_start + per_rank_repeat))
    return indices
...
def _compute_delta_opd_sampled_tensors(
    teacher_sampled_log_probs: torch.Tensor,
    teacher_ref_sampled_log_probs: torch.Tensor,
    response_mask: torch.Tensor,
) -> dict[str, torch.Tensor]:
    if teacher_sampled_log_probs.shape != teacher_ref_sampled_log_probs.shape:
        raise ValueError(
            "teacher_sampled_log_probs and teacher_ref_sampled_log_probs must have the same shape, "
            f"got {teacher_sampled_log_probs.shape=} and {teacher_ref_sampled_log_probs.shape=}"
        )
    if teacher_sampled_log_probs.shape != response_mask.shape:
        raise ValueError(
            "sampled teacher log-probs and response_mask must have the same shape, "
            f"got {teacher_sampled_log_probs.shape=} and {response_mask.shape=}"
        )

    sampled_delta = (teacher_sampled_log_probs.detach() - teacher_ref_sampled_log_probs.detach()).detach()
    sampled_rm_scores = (sampled_delta * response_mask).detach()
    return {
        "rm_scores": sampled_rm_scores,
        "delta_opd_sampled_log_ratio": sampled_delta,
        "teacher_ref_sampled_log_probs": teacher_ref_sampled_log_probs.detach(),
    }


def _merge_delta_opd_sampled_tensors(batch: DataProto, sampled_tensors: dict[str, torch.Tensor]) -> DataProto:
    if "rm_scores" in batch.batch.keys() and "rm_scores" in sampled_tensors:
        batch.batch.pop("rm_scores")
    return batch.union(DataProto.from_dict(tensors=sampled_tensors))


def _compute_delta_opd_sampled_metrics(
    sampled_delta: torch.Tensor,
    rm_scores: torch.Tensor,
    response_mask: torch.Tensor,
    chunk_size: int = 1024,
) -> dict:
    if sampled_delta.shape != rm_scores.shape or sampled_delta.shape != response_mask.shape:
        raise ValueError(
            "sampled_delta, rm_scores, and response_mask must have the same shape, "
            f"got {sampled_delta.shape=}, {rm_scores.shape=}, {response_mask.shape=}"
        )
    sampled_mask = response_mask.bool()
    valid_sampled_delta = sampled_delta[sampled_mask]
    valid_rm_scores = rm_scores[sampled_mask]
    if valid_sampled_delta.numel() == 0:
        return {}

    metrics = {
        "delta_opd_sampled/log_ratio_mean": valid_sampled_delta.mean().detach().item(),
        "delta_opd_sampled/log_ratio_std": valid_sampled_delta.std(unbiased=False).detach().item(),
        "delta_opd_sampled/log_ratio_pos_frac": (valid_sampled_delta > 0).float().mean().detach().item(),
        "delta_opd_sampled/log_ratio_abs_mean": valid_sampled_delta.abs().mean().detach().item(),
        "delta_opd_sampled/log_ratio_min": valid_sampled_delta.min().detach().item(),
        "delta_opd_sampled/log_ratio_max": valid_sampled_delta.max().detach().item(),
        "delta_opd_sampled/reward_mean": valid_rm_scores.mean().detach().item(),
        "delta_opd_sampled/reward_std": valid_rm_scores.std(unbiased=False).detach().item(),
        "delta_opd_sampled/reward_abs_mean": valid_rm_scores.abs().mean().detach().item(),
        "delta_opd_sampled/reward_min": valid_rm_scores.min().detach().item(),
        "delta_opd_sampled/reward_max": valid_rm_scores.max().detach().item(),
        "delta_opd_sampled/reward_token_sum_mean": (rm_scores * response_mask).sum(dim=-1).mean().detach().item(),
    }
    ...
    for start in range(0, seq_len, chunk_size):
        ...
        chunk_mask = sampled_mask[:, start:end]
        if not chunk_mask.any():
            continue
        chunk_delta = sampled_delta[:, start:end][chunk_mask]
        chunk_rewards = rm_scores[:, start:end][chunk_mask]
        chunk_reward_sum = (rm_scores[:, start:end] * response_mask[:, start:end]).sum(dim=-1)
        metrics[f"delta_opd_sampled/log_ratio_mean_chunk_{start}_{end}"] = chunk_delta.mean().detach().item()
        metrics[f"delta_opd_sampled/log_ratio_pos_frac_chunk_{start}_{end}"] = (
            (chunk_delta > 0).float().mean().detach().item()
        )
        metrics[f"delta_opd_sampled/reward_mean_chunk_{start}_{end}"] = chunk_rewards.mean().detach().item()
        metrics[f"delta_opd_sampled/reward_token_sum_mean_chunk_{start}_{end}"] = (
            chunk_reward_sum.mean().detach().item()
        )
...
class RayPPOTrainer:
    ...
    def __init__(
        ...
        self.use_delta_opd_sampled = self.reward_mode == "delta_opd_sampled"
        self.use_teacher_ref_scorer = self.use_delta_opd or self.use_delta_opd_sampled
    ...
    def _dump_generations(self, inputs, outputs, gts, scores, reward_extra_infos_dict, dump_path):
        ...
        # Async upload to HDFS if configured.
        remote_dst = self._remote_outputs_path_for(filename)
        if remote_dst is not None:
            get_uploader().enqueue_copy(filename, remote_dst)

    def _remote_outputs_path_for(self, local_filename: str) -> Optional[str]:
        """Map a local file path to its HDFS counterpart using trainer.hdfs_upload.* config.

        Returns None when no mapping is configured.
        """
        hdfs_upload_cfg = self.config.trainer.get("hdfs_upload", None)
        if hdfs_upload_cfg is None:
            return None
        local_root = hdfs_upload_cfg.get("local_outputs_dir", None)
        remote_root = hdfs_upload_cfg.get("remote_outputs_dir", None)
        if not local_root or not remote_root:
            return None
        import posixpath

        abs_local = os.path.abspath(local_filename)
        abs_root = os.path.abspath(local_root)
        if not abs_local.startswith(abs_root.rstrip(os.sep) + os.sep) and abs_local != abs_root:
            return None
        rel = os.path.relpath(abs_local, abs_root)
        rel_posix = rel.replace(os.sep, "/")
        return posixpath.join(remote_root.rstrip("/"), rel_posix)
    ...
    def _validate(self):
        ...
        for test_data in self.val_dataloader:
            ...
            per_prompt_balanced_indices = _build_per_prompt_balanced_indices(
                num_prompts=num_prompts,
                repeat_times=val_repeat_times,
                dp_size=rollout_dp_size,
            )
            if per_prompt_balanced_indices is not None:
                test_batch.reorder(torch.tensor(per_prompt_balanced_indices))
            else:
                print(
                    "Skip per-prompt validation balancing because "
                    f"val_n={val_repeat_times} is not divisible by rollout_dp_size={rollout_dp_size}"
                )
    ...
    def _save_checkpoint(self):
        ...
        if self.config.trainer.default_hdfs_dir is not None:
            remote_global_step_folder = os.path.join(
        ...
        if self.use_critic:
            ...
            if remote_global_step_folder is not None:
                critic_remote_path = os.path.join(remote_global_step_folder, str(Role.Critic))
        ...
        # Worker checkpoint managers upload FSDP shards from every rank. The
        # driver only uploads driver-owned state.
        if self.config.trainer.default_hdfs_dir is not None:
            assert remote_global_step_folder is not None
            get_uploader().enqueue_copy(
                dataloader_local_path,
                os.path.join(remote_global_step_folder, os.path.basename(dataloader_local_path)),
            )
            remote_latest_checkpointed_iteration = os.path.join(
                self.config.trainer.default_hdfs_dir, "latest_checkpointed_iteration.txt"
            )
            get_uploader().enqueue_copy(local_latest_checkpointed_iteration, remote_latest_checkpointed_iteration)
    ...
    def _resolve_auto_resume_checkpoint_dir(self) -> str:
        checkpoint_folder = self.config.trainer.default_hdfs_dir or self.config.trainer.default_local_dir
    ...
    def _normalize_checkpoint_path(path: Optional[str]) -> Optional[str]:
        if path is None:
            return None
        if is_non_local(path) or os.path.isabs(path):
            return path
    ...
    def _find_latest_resume_checkpoint(self, checkpoint_folder: Optional[str]) -> Optional[str]:
        if checkpoint_folder is None:
            return None
        if is_non_local(checkpoint_folder):
            return self._find_latest_hdfs_checkpoint(checkpoint_folder)
        return find_latest_ckpt_path(checkpoint_folder)

    def _find_latest_hdfs_checkpoint(self, checkpoint_folder: str) -> Optional[str]:
        tracker_path = os.path.join(checkpoint_folder, "latest_checkpointed_iteration.txt")
        if not hdfs_io.exists(tracker_path):
            print(f"Checkpoint tracker file does not exist: {tracker_path}")
            return None

        local_tracker_path = copy_to_local(tracker_path, always_recopy=True)
        with open(local_tracker_path, "rb") as f:
            iteration = int(f.read().decode())

        while iteration >= 0:
            ckpt_path = os.path.join(checkpoint_folder, f"global_step_{iteration}")
            if hdfs_io.exists(ckpt_path):
    ...
    def _resolve_checkpoint_file(self, checkpoint_root: str, relative_path: str) -> str:
        remote_or_local_path = os.path.join(checkpoint_root, relative_path)
        if is_non_local(remote_or_local_path):
            return copy_to_local(remote_or_local_path, always_recopy=True)
        return remote_or_local_path
                ...
                with marked_timer("step", timing_raw):
                    ...
                    with marked_timer("reward", timing_raw, color="yellow"):
                        ...
                        if self.use_rm and "rm_scores" not in batch.batch.keys():
                            ...
                            if reward_mode == "delta_opd" and top_k <= 0:
                                raise ValueError("reward_mode=delta_opd requires actor_rollout_ref.rollout.log_prob_top_k > 0")
                            ...
                            if reward_mode == "delta_opd_sampled":
                                if self.teacher_ref_rm_wg is None:
                                    raise ValueError("reward_mode=delta_opd_sampled requires teacher_ref_rm_wg")
                                ...
                                if "teacher_sampled_log_probs" not in batch.batch.keys():
                                    raise ValueError(
                                        "rm_wg.compute_rm_score must return teacher_sampled_log_probs "
                                        "for reward_mode=delta_opd_sampled"
                                    )
                                if "teacher_sampled_log_probs" not in teacher_ref_raw.batch.keys():
                                    raise ValueError(
                                        "teacher_ref_rm_wg.compute_rm_score must return teacher_sampled_log_probs "
                                        "for reward_mode=delta_opd_sampled"
                                    )

                                sampled_tensors = _compute_delta_opd_sampled_tensors(
                                    teacher_sampled_log_probs=batch.batch["teacher_sampled_log_probs"],
                                    teacher_ref_sampled_log_probs=teacher_ref_raw.batch["teacher_sampled_log_probs"],
                                    response_mask=batch.batch["response_mask"],
                                )
                                if "teacher_entropy" in teacher_ref_raw.batch.keys():
                                    sampled_tensors["teacher_ref_entropy"] = teacher_ref_raw.batch["teacher_entropy"]
                                if "teacher_on_student_log_probs" in teacher_ref_raw.batch.keys():
                                    sampled_tensors["teacher_ref_on_student_log_probs"] = teacher_ref_raw.batch[
                                        "teacher_on_student_log_probs"
                                    ]
                                if "overlap_mask" in teacher_ref_raw.batch.keys():
                                    sampled_tensors["teacher_ref_overlap_mask"] = teacher_ref_raw.batch[
                                        "overlap_mask"
                                    ]
                                batch = _merge_delta_opd_sampled_tensors(batch, sampled_tensors)

                                response_mask = batch.batch["response_mask"]
                                metrics.update(
                                    _compute_delta_opd_sampled_metrics(
                                        sampled_delta=batch.batch["delta_opd_sampled_log_ratio"],
                            ...
                            elif reward_mode == "delta_opd":
                                if self.teacher_ref_rm_wg is None:
                                    raise ValueError("reward_mode=delta_opd requires teacher_ref_rm_wg")
                                with marked_timer("compute_teacher_ref_rm_score", timing_raw, color="magenta"):
                                    teacher_ref_raw = self.teacher_ref_rm_wg.compute_rm_score(batch)
                        ...
                        if "swanlab" in self.config.trainer.logger and (self.global_steps == 1 or self.global_steps % 10 == 0) and "student_valid_counts" in batch.batch.keys():
                            try:
                    ...
                    if self.config.trainer.get("is_plot", False) and "swanlab" in self.config.trainer.logger and (self.global_steps == 1 or self.global_steps % 10 == 0):
                        try:
                            import matplotlib.pyplot as plt
                            import swanlab
