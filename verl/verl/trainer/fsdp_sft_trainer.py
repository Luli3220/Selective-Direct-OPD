...
class FSDPSFTTrainer:
    ...
    def save_checkpoint(self, step):
        ...
        if getattr(self.config.trainer, "default_hdfs_dir", None):
            remote_global_step_folder = os.path.join(self.config.trainer.default_hdfs_dir, f"global_step_{step}")

        # Use checkpoint manager to save. The remote path is passed through so
        # each rank uploads its own shard in multi-node runs.
        self.checkpoint_manager.save_checkpoint(
            local_path=local_global_step_folder,
            hdfs_path=remote_global_step_folder,
            global_step=step,
        ...
        # Copy driver-owned state to HDFS. Model/optimizer/extra shards are
        # uploaded by the checkpoint manager from every rank.
        if getattr(self.config.trainer, "default_hdfs_dir", None) and self.device_mesh.get_rank() == 0:
            ...
            get_uploader().enqueue_copy(
                tracker_file,
                os.path.join(self.config.trainer.default_hdfs_dir, os.path.basename(tracker_file)),
            )
...
def run_sft(config):
    ...
    from verl.utils.async_hdfs_uploader import init_uploader, shutdown_uploader

    hdfs_upload_cfg = config.trainer.get("hdfs_upload", None) if hasattr(config, "trainer") else None
    init_uploader(hdfs_upload_cfg)
    try:
        trainer.fit()
    finally:
        failures = shutdown_uploader(final_wait=True)
        fail_on_error = bool(hdfs_upload_cfg.get("fail_on_error", False)) if hdfs_upload_cfg is not None else False
        destroy_global_process_group()
        if failures and fail_on_error:
            raise RuntimeError(f"{len(failures)} HDFS upload task(s) failed; first={failures[0]}")
