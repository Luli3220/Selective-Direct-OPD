...
# Keep this random-token ablation in a separate checkpoint/log directory.
export PYTHON_BIN=${PYTHON_BIN:-$(command -v python)}
export CUDA_VISIBLE_DEVICES=${CUDA_VISIBLE_DEVICES:-0,1,2,3,4,5,6,7}
export GPUS_PER_NODE=${GPUS_PER_NODE:-8}
export NUM_NODES=${NUM_NODES:-1}
export EXPERIMENT_NAME=${EXPERIMENT_NAME:-justrl_qwen3_1p7b_random_top10_seed42_kl}
export JS_TOKEN_FILTER_ENABLED=${JS_TOKEN_FILTER_ENABLED:-True}
export JS_TOP_FRACTION=${JS_TOP_FRACTION:-0.10}
export JS_TOKEN_SELECTION_MODE=${JS_TOKEN_SELECTION_MODE:-random}
export JS_TOKEN_SELECTION_SEED=${JS_TOKEN_SELECTION_SEED:-42}
export USE_KL_LOSS=${USE_KL_LOSS:-True}
export ADAPTIVE_KL_LOSS_COEF=${ADAPTIVE_KL_LOSS_COEF:-True}

# Match the 8 x H200 top-JS baseline training and validation settings.
